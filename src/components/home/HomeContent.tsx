  ...
  return (
    <div className="bg-light min-h-screen relative">
      ...
      <div className="h-screen flex justify-center flex-col  items-center  ">
          ...
          {["Sajjad", "jonayed"].map((text, index) => (
            <span className="space-x-[0.5vw]" key={index}>
              {text.split("").map((char, i) => (
                <span
                  key={i}
                  className={`inline-block  ${i > 0 && "text-animation "}`}
                >
                  {char}
                </span>
              ))}
        ...
        <div className=" z-20">
          <p className=" lg:text-[2vw] md:text-[4vw] text-[5vw]   font-body uppercase  tracking-wider  font-light text-dark bg overflow-hidden flex justify-center">
            <span className="relative top-[6vh]  body-text ">
              fullstack web developer
            </span>
          </p>
          <p className="lg:text-[1vw] text-[4vw]  md:text-[4vw]  mt-[0.8vh] text-center font-body uppercase font-light text-dark bg overflow-hidden lg:tracking-[0.7vw] md:tracking-[1.5vw] tracking-[2vw]">
            <span className="relative top-[6vh] body-text">
              based in bangladesh
            </span>
          </p>
          <div className="mx-auto relative w-max mt-8 flex gap-4">
            <div className="bg-light absolute w-full h-full z-30 right-0 customBtn"></div>
